...
const AccessRestrictedScreen = ({
  ...
  /**
   * Maps a restriction call-to-action onto a route.
   *
   * `go_back` deliberately uses `navigation.goBack()` on every platform:
   * React Navigation's linking integration owns the browser history on web,
   * so calling `window.history.back()` here desynchronised the two.
   */
  const ACTION_ROUTES = {
    edit_profile: 'OrganizationProfile',
    upload_contract: 'OrganizationContract',
